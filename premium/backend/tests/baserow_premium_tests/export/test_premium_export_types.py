...
def test_excel_export_strips_xml_illegal_control_characters(
    ...
    # A cell (or field name) carrying an XML-illegal control character must not
    # abort the whole Excel export with openpyxl's IllegalCharacterError. The
    # illegal byte should be stripped/replaced while allowed whitespace
    # (\t, \n, \r) is preserved. A "="-leading value with an illegal byte must
    # also be handled (formula-injection branch still goes through check_string).
    storage_mock = MagicMock()
    get_storage_mock.return_value = storage_mock
    user = premium_data_fixture.create_user(has_active_premium_license=True)
    table = premium_data_fixture.create_database_table(user=user)
    ...
    worksheet = load_workbook(filename=BytesIO(wb_bytes)).active
    # Header: illegal byte stripped.
    assert worksheet.cell(row=1, column=1).value == "Notes"
    # Row values: illegal bytes stripped, allowed whitespace preserved.
